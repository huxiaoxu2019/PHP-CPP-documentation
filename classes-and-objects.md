...
```
默认情况下，每一个方法（我面我们会提到的属性也同样）是公开的（public）。你可以通过传递一个可选的参数Php::protected或者Php::private来标记某个方法是公开的还是私有的。如果你想要抽象的或者最终的（Final）的方法，那么可以将传递的参数值设定为Php::Abstract或Php::Final。我们将上述的value()方法设定为Final类型的，比保证不能够被子类重写。
